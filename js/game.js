...
function start() {
	audio.play("../media/game/bgm.mp3", { volume: 0.1, loop: true });
	...
	const loop = (time) => {
		let TotalPoints = game.points + game.coins;
		...
		setTimeout(() => {
			...
			if (game.points / 300 + 1 === game.level && game.level > 1) {
				if (time > game.currentTime + 2000) {
					audio.play("../media/game/levelnext.mp3", { volume: 0.1 });
				...
				switch (object.type) {
					case "coin":
						object.onCollision(player, () => {
							...
							audio.play("../media/game/coin.mp3", { volume: 0.1 });
